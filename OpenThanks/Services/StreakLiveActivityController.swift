...
/// running until the person posts again or local midnight ends the streak.
@MainActor
enum StreakLiveActivityController {
    ...
    /// Best-effort end at midnight while the process is still alive.
    private static func scheduleLocalEnd(at deadline: Date) {
        let delay = deadline.timeIntervalSinceNow
        guard delay > 0, delay < 48 * 3600 else { return }
        Task {
            try? await Task.sleep(for: .seconds(delay))
            // Re-check — only end if they still haven't posted.
            for activity in Activity<StreakLiveActivityAttributes>.activities {
                if activity.content.state.postedToday { continue }
                if Date() >= activity.content.state.deadline {
                    await activity.end(nil, dismissalPolicy: .immediate)
