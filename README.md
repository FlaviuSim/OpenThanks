...
- URL `https://dsftvyuzmhlqadhbubgw.supabase.co` and publishable key `sb_publishable_6E3JYy1bWx45o9o9NmDjbg_77tAjxN1` (both in `Config/AppConfig.swift`)
...
3. **Auth providers:** enable Apple in Supabase Auth → Providers (native flow, no secret needed), confirm Google is enabled, and add `openthanks://auth-callback` to Auth → URL Configuration → Redirect URLs. LinkedIn from the web design was intentionally dropped — Apple replaces it, and App Review requires Sign in with Apple once any third-party login is offered.
